...
\tikzstyle{block2} = [rectangle, draw, fill=RougeCEA, text width=10em, text centered, minimum height=2.5em, thick]
\tikzstyle{block3} = [rectangle, draw, fill=BleuCLMPS, text width=5em, text centered, minimum height=2.5em, thick]
...
\begin{tikzpicture}[remember picture,node distance = 2cm]
	...
	\begin{scope}[]
		...
		\fill[BleuCLMPS] ($(Database.north west)+(-0.3,0.4)$) rectangle ($(Latin.south east)+(0.5,-0.3)$); 
	...
	\matrix [column sep=7mm,row sep=12mm]
	{
		...
		& \node [block_light] (Nucl) {Nuclear simulation};\\
		& \node [block_light] (th) {Thermal simulation};\\
		& \node [block_light] (Elas) {Quasi-static elastic mechanical simulation}; & \node [block2] (fr) {\textbf{\textcolor{white}{Fragility curves}}}; \\
		& \node [block_light] (Post) {Wöhler post processing of the elastic stress };\\
		& \node [block_blue] (Latin) {\textbf{\textcolor{BleuLMPS!10}{Enhanced LATIN solver}}};\\
	};
