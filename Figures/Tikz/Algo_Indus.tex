...
\tikzstyle{block} = [rectangle, draw, fill=BleuLMPS!80, text width=15em, text centered, minimum height=2.5em, thick]
...
\tikzstyle{block2} = [rectangle, draw, fill=BleuLMPS!80, text width=10em, text centered, minimum height=2.5em, thick]
\tikzstyle{block3} = [rectangle, draw, fill=BleuLMPS!80, text width=5em, text centered, minimum height=2.5em, thick]
...
\begin{tikzpicture}[remember picture,node distance = 2cm]
		...
		\matrix [column sep=7mm,row sep=12mm]
	{
		...
		& \node [block] (Latin) {Enhanced LATIN solver};\\
	};
	...
	\matrix [column sep=7mm,row sep=12mm]
	{
		...
		& \node [block_light] (Nucl) {Nuclear simulation};\\
		& \node [block_light] (th) {Thermal simulation};\\
		& \node [block_light] (Elas) {Quasi-static elastic mechanical simulation}; & \node [block2] (fr) {\textcolor{BleuLMPS!10}{\textbf{Fragility curves}}}; \\
		& \node [block_light] (Post) {Wöhler post processing of the elastic stress };\\
		& \node [block] (Latin) {\textcolor{BleuLMPS!10}{\textbf{Enhanced LATIN solver}}};\\
	};
...
%	\path [line] (Indexed.east) -- node [midway,above] {$\left\{ \vect{u}^{(i)}_d(t)\right\}$} (Latin.west);
	  ...
	  \node [block3, left = 2cm of Latin] (Database) {\textcolor{BleuLMPS!10}{\textbf{Data base}}};
