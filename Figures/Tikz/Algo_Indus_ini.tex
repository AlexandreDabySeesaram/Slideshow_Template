...
\tikzstyle{block_light} = [rectangle, draw, fill=white, text width=15em, text centered, minimum height=2.5em, thick]
...
\begin{tikzpicture}[remember picture,node distance = 2cm]
	...
	\begin{scope}[]
		\fill[white] ($(Nucl.north west)+(-4,0.3)$) rectangle ($(Post.south east)+(0.5,-0.3)$);
